...
class main_node():
    def __init__(self):
        ...
        self.debug_yolo = False
        self.debug_obstacle_avoidance = True
    ...
    def speed_control(self):
        if(self.cmd.brake > 1 and self.cmd.brake <= 33):
            ...
            speed = abs(self.goal.speed + (self.goal.speed-self.status.speed)*2)
    ...
    def brake_control(self):
        if(self.goal.speed == 0 and self.status.speed == 0):
            ...
            if(error < 0):
                brake = (abs(error)+10) * self.brake_gain
            elif(error >= 0):
                brake = 1
